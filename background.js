...
function doButtons(tab) {
console.log(tab.url);
	
if (tab.url.match(/cmd=showCamp/) && tab.url.match(/cid=/)) {

	randomString = rndString(17);
	
	cid = tab.url.match(/&cid=([0-9]+)/)[1];
	
	ulogin = tab.url.match(/&ulogin=([^\&]+)/);
...
else if (tab.url.match(/cmd=showCamps/) && tab.url.match(/ulogin=/)) {
	console.log('Мы на странице всех кампаний, щас покажем кнопочки к каждой');
}
else if (tab.url.match(/cmd=transfer/) && tab.url.match(/ulogin=/)){
